...
cytoscape.use(fcose)
const props = defineProps<{ data: Atlas; selected: string; group: string; labels: boolean; neighborhood: boolean; layout: string; theme: string; pinnedId: string }>()
const emit = defineEmits<{ select: [kind: 'character' | 'relation' | 'none', id: string]; zoom: [value: number]; ready: []; hover: [id: string]; leave: []; pin: [id: string] }>()
const container = ref<HTMLDivElement>()
const hoverNode = ref(''), pinButtons = ref<{id:string;x:number;y:number}[]>([])
let pinTimer:ReturnType<typeof setTimeout>
function updatePins(){if(!cy)return;pinButtons.value=[...new Set([hoverNode.value,props.pinnedId].filter(Boolean))].flatMap(id=>{const node=cy.getElementById(`c:${id}`);if(!node.length||!node.visible())return [];const p=node.renderedPosition(),r=node.renderedWidth()/2;return [{id,x:p.x+r*.73,y:p.y-r*.73}]})}
function holdPin(){clearTimeout(pinTimer);if(hoverNode.value)emit('hover',hoverNode.value)}
function leavePin(){clearTimeout(pinTimer);emit('leave');pinTimer=setTimeout(()=>{hoverNode.value='';updatePins()},400)}
function pointerMove(event:PointerEvent){
  if(!cy||!container.value||event.buttons)return
  if((event.target as Element).closest('.node-pin')){holdPin();return}
  const rect=container.value.getBoundingClientRect(),x=event.clientX-rect.left,y=event.clientY-rect.top
  const node=cy.nodes(':visible').toArray().find(n=>{const p=n.renderedPosition();return Math.hypot(x-p.x,y-p.y)<=n.renderedWidth()/2+2})
  if(node?.length){clearTimeout(pinTimer);const id=node.data('rawId');if(hoverNode.value!==id){hoverNode.value=id;updatePins();emit('hover',id)}}
...
function sync() {
  ...
  cy.batch(() => {
    cy.elements().remove()
    cy.add(props.data.characters.map(c => ({ group: 'nodes' as const, data: { id: `c:${c.id}`, label: c.name, color: c.color || '#4f8072', group: c.group, rawId: c.id }, position: positions.get(`c:${c.id}`) })))
    cy.add(props.data.relations.map(r => ({ group: 'edges' as const, data: { id: `r:${r.id}`, source: `c:${r.from}`, target: `c:${r.to}`, label: relationLabel(r), rawId: r.id }, classes: r.direction === 'two-way' ? 'two-way' : '' })))
  })
  applyVisibility()
...
onMounted(() => {
  ...
  cy.on('pan zoom position layoutstop',updatePins)
...
function applyTheme(){if(!cy||!container.value)return;const css=getComputedStyle(container.value),get=(key:string)=>css.getPropertyValue(key).trim();cy.style().selector('node').style({'background-color':get('--node-bg'),color:get('--text')}).selector('edge').style({'line-color':get('--edge'),'target-arrow-color':get('--edge'),'source-arrow-color':get('--edge'),color:get('--muted'),'text-background-color':get('--paper')}).selector('node.chosen').style({'background-color':get('--soft'),'underlay-color':get('--green')}).selector('edge.chosen').style({'line-color':get('--green'),color:get('--green'),'target-arrow-color':get('--green'),'source-arrow-color':get('--green')}).update()}
watch(()=>props.theme,async()=>{await nextTick();applyTheme()})
watch(()=>props.pinnedId,updatePins)
...
async function exportImage():Promise<Blob>{
  if(!cy||!props.data.characters.length)throw new Error('请先添加角色，再导出关系网图片。')
  const host=document.createElement('div');host.style.cssText='position:fixed;left:-20000px;top:0;width:1600px;height:1200px;';document.body.append(host)
  let exported:Core|undefined
  try{
    const css=getComputedStyle(container.value!),background=css.getPropertyValue('--paper').trim()
    const elements:cytoscape.ElementDefinition[]=cy.elements().map(item=>({data:item.data(),position:item.isNode()?item.position():undefined,group:item.group(),classes:item.hasClass('two-way')?'two-way':''}))
    exported=cytoscape({container:host,elements,style:cy.json().style,layout:{name:'preset'},pixelRatio:1})
    exported.elements().removeClass('hidden dimmed chosen no-label')
    exported.layout({name:'fcose',animate:false,randomize:true,nodeDimensionsIncludeLabels:true,idealEdgeLength:125,nodeRepulsion:12000,numIter:1500,tile:true} as LayoutOptions).run()
    const boxes=exported.nodes().map(n=>{const b=n.boundingBox({includeLabels:true,includeOverlays:false,includeUnderlays:false});return {id:n.id(),x:(b.x1+b.x2)/2,y:(b.y1+b.y2)/2,width:b.w,height:b.h}})
    separateBoxes(boxes).forEach((box,i)=>{const n=exported!.getElementById(box.id),p=n.position();n.position({x:p.x+box.x-boxes[i]!.x,y:p.y+box.y-boxes[i]!.y})})
    await document.fonts.ready
    const bounds=exported.elements().boundingBox(),scale=Math.min(2,8000/Math.max(bounds.w,bounds.h),Math.sqrt(32_000_000/(bounds.w*bounds.h)))
    return await exported.png({output:'blob-promise',full:true,bg:background,scale}) as Blob
  }finally{exported?.destroy();host.remove()}
}
defineExpose({ arrange, focus, zoomBy, fit, exportImage })
onBeforeUnmount(() => { clearTimeout(pinTimer);activeLayout?.stop(); observer?.disconnect(); cy?.destroy() })
</script>
<template><div class="graph-canvas" @pointermove="pointerMove" @pointerleave="leavePin"><div ref="container" class="graph-engine" role="img" aria-label="可交互角色关系图，悬停角色查看关系，滚轮缩放，拖动平移" /><button v-for="pin in pinButtons" :key="pin.id" class="node-pin" :class="{ pinned: pinnedId===pin.id }" :style="{left:pin.x+'px',top:pin.y+'px'}" :aria-label="`${pinnedId===pin.id?'取消固定':'固定'}${data.characters.find(c=>c.id===pin.id)?.name}的关系窗口`" :title="pinnedId===pin.id?'取消固定':'固定关系窗口'" @mouseenter="holdPin" @mouseleave="leavePin" @click.stop="emit('pin',pin.id)"><PinOff v-if="pinnedId===pin.id" :size="13"/><Pin v-else :size="13"/></button></div></template>
