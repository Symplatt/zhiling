  ...
  <aside class="inspector">
    ...
    <div class="inspector-scroll">
      <template v-if="character">
        ...
        <p class="profile-memberships">{{ characterGroups(character).join(' · ') || '未分组' }}</p>
          ...
          >
            <span class="relation-direction"
              ><ArrowLeftRight
                ...
                v-else
                :size="15"
                :class="{ reversed: r.from === character.id }" /></span
            ><span
              ><strong>{{
                name(r.from === character.id ? r.to : r.from)
              }}</strong
              ><small class="relation-endpoints"
                ><SelfReference v-if="r.from === r.to" :name="character.name" />
                <span v-else class="endpoint-name">{{
                  name(r.from === character.id ? r.to : r.from)
                }}</span>
                <span>{{
                  r.direction === "two-way"
                ...
                <SelfReference :name="character.name" /></small></span
    ...
    <div v-if="character || relation" class="inspector-foot">
      <template v-if="character">
        <button
          class="text-button"
