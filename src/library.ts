...
export const themes: { id: Theme; name: string; description: string; color: string; background: string }[] = [
  { id: 'mono', name: '黑白灰', description: '留白与秩序', color: '#555b64', background: '#f4f4f5' },
  { id: 'grass', name: '浅草', description: '草木间的清新', color: '#4c7655', background: '#f1f6eb' },
  { id: 'pink', name: '桃粉', description: '温柔的春日', color: '#b66683', background: '#fff1f5' },
  { id: 'blue', name: '碧蓝', description: '晴空与远海', color: '#347eaa', background: '#ecf7ff' },
...
export interface Library { version: 2; activeId: string; theme: Theme; graphs: Story[] }
...
export function parseLibrary(input: unknown): Library {
  ...
  return { version: 2, activeId: ids.has(String(raw.activeId)) ? String(raw.activeId) : graphs[0]!.id, theme: themes.some(t => t.id === raw.theme) ? raw.theme as Theme : 'grass', graphs }
}
