...
const root=path.resolve(__dirname,'../test-results/zhiling-desktop'),profile=path.join(root,'profile')
...
app.on('browser-window-created',(_,win)=>{win.hide();win.webContents.once('did-finish-load',async()=>{
  try{
    ...
    await win.webContents.executeJavaScript(`Array.from(document.querySelectorAll('button')).find(b=>b.textContent.trim()==='导出图片').click()`)
